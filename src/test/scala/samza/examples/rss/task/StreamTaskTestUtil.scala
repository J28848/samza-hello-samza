...
object StreamTaskTestUtil {
  ...
  props1.setProperty("auto.create.topics.enable","true")
  props2.setProperty("auto.create.topics.enable","true")
  props3.setProperty("auto.create.topics.enable","true")
 ...
 */
abstract class StreamTaskTestUtil extends JUnitSuite{
  ...
  def send(msg: String) {
    ...
    MyTestTask.awaitMessage
    assertEquals(msg, MyTestTask.received.last)
  }
...
object MyTestTask {
  val tasks = new HashMap[TaskName, StreamTask] with SynchronizedMap[TaskName, StreamTask]
  var totalTasks = 1
  ...
  def reset(): Unit = {
    MyTestTask.totalTasks = StreamTaskTestUtil.TOTAL_TASK_NAMES
    MyTestTask.allTasksRegistered = new CountDownLatch(MyTestTask.totalTasks)
  }
  ...
  /**
    ...
    * tasks.
    */
  def register(taskName: TaskName, task: StreamTask) {
    tasks += taskName -> task
